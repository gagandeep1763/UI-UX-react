...
const AboutSection = ({ onNavigate }) => {
  ...
  const [philosophyRef, isPhilosophyVisible] = useScrollAnimation({ threshold: 0.1 });
  const [experienceRef, isExperienceVisible] = useScrollAnimation({ threshold: 0.1 });
  const [learningRef, isLearningVisible] = useScrollAnimation({ threshold: 0.1 });
    ...
    <section id="about" className="py-20 bg-gradient-to-b from-background to-secondary/30">
      <div className="container px-4 mx-auto">
        <div className="max-w-4xl mx-auto">
          ...
          <div
            ref={introRef}
            className={`transition-all duration-1000 ${isIntroVisible ? 'animate-slide-up' : 'opacity-0'}`}
          >
          ...
          <div
            ref={philosophyRef}
            className={`mb-16 transition-all duration-1000 ${isPhilosophyVisible ? 'animate-slide-up' : 'opacity-0'}`}
          >
            ...
            <div className="grid md:grid-cols-3 gap-6">
              {principles.map((principle, index) => (
                <div
                  key={index}
                  className={`transition-all duration-1000 ${isPhilosophyVisible ? 'animate-slide-up' : 'opacity-0'}`}
                  style={{ transitionDelay: `${index * 200}ms` }}
                >
                  <Card
                    className="border-0 shadow-md bg-white/60 backdrop-blur-sm hover:bg-white/80 transition-all duration-300 hover:scale-105 h-full"
                  >
          ...
          <div
            ref={experienceRef}
            className={`mb-12 transition-all duration-1000 ${isExperienceVisible ? 'animate-slide-up' : 'opacity-0'}`}
          >
            <h3 className="text-2xl font-semibold text-center mb-8 text-foreground">
              Project Experience
            </h3>
            <div className="grid md:grid-cols-2 gap-6">
              {experiences.map((exp, index) => (
                <div
                  key={index}
                  className={`transition-all duration-1000 ${isExperienceVisible ? 'animate-slide-up' : 'opacity-0'}`}
                  style={{ transitionDelay: `${index * 200}ms` }}
                >
                  <Card
                    className="border-0 shadow-md bg-white/60 backdrop-blur-sm hover:bg-white/80 transition-all duration-300 hover:scale-105 h-full"
          ...
          <div
            ref={learningRef}
            className={`transition-all duration-1000 ${isLearningVisible ? 'animate-slide-up' : 'opacity-0'}`}
          >
